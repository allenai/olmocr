...
from olmocr.work_queue import GCSBackend, LocalBackend, S3Backend, WorkQueue
...
async def main():
    ...
    # Beaker/cluster and GCP job running
    from olmocr.beaker_runner import add_beaker_args
    from olmocr.gcp_runner import add_gcp_args

    add_beaker_args(parser)
    add_gcp_args(parser)
    ...
    from olmocr.beaker_runner import setup_beaker_environment

    if await setup_beaker_environment():
        workspace_s3 = boto3.client("s3")
        pdf_s3 = boto3.client("s3")
    ...
    workspace_gcs = None
    if args.workspace.startswith("gs://"):
    ...
    elif args.workspace.startswith("gs://"):
        work_queue = WorkQueue(GCSBackend(workspace_gcs, args.workspace))
    else:
    ...
    if args.pdfs:
        ...
        for pdf_path in args.pdfs:
            # Expand s3/gcs glob paths first, then categorize results
            if pdf_path.startswith("s3://"):
                ...
                raise ValueError("pdfs argument needs to be either a local path, an s3 path, a gs:// path, or a glob pattern...")
    ...
    if args.beaker:
        from olmocr.beaker_runner import submit_beaker_job

        submit_beaker_job(args)
        return

    if args.gcp:
        from olmocr.gcp_runner import submit_gcp_job

        submit_gcp_job(args, unknown_args)
        return
