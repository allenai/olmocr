...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: $0 [--config CONFIG_PATH] [--gpus N] [--skip-docker-build] [--preemptible]"
            exit 1
            ;;
    esac
done
...
cat << 'EOF' > /tmp/run_training_experiment_multi.py
import sys
...
# Get image tag, beaker user, git branch, git hash, config, preemptible, and gpu count from command line
...
# Build the training command (torchrun for multi-GPU single-node)
commands = [
    "pip install .[train]",
    ...
    f"torchrun --nnodes=1 --nproc_per_node={gpu_count} -m olmocr.train.train --config {config}"
]

# Build task spec
task_spec = TaskSpec(
    name="olmocr-training-mgpu",
    ...
    env_vars=[
        ...
        # Some NCCL defaults that tend to be safe in multi-GPU single node
        EnvVar(name="NCCL_P2P_DISABLE", value="1"),
        EnvVar(name="NCCL_IB_DISABLE", value="1"),
    ]
)

# Create experiment spec
experiment_spec = ExperimentSpec(
    description=f"OlmOCR Training Run (multi-GPU) - Branch: {git_branch}, Commit: {git_hash}",
    budget="ai2/oe-base",
    tasks=[task_spec],
)
...
echo "Creating Beaker multi-GPU experiment..."
$PYTHON /tmp/run_training_experiment_multi.py $IMAGE_TAG $BEAKER_USER $GIT_BRANCH $GIT_HASH "$CONFIG" $PREEMPTIBLE $GPUS

# Clean up temporary file
rm /tmp/run_training_experiment_multi.py

echo "Multi-GPU training experiment submitted successfully!"
